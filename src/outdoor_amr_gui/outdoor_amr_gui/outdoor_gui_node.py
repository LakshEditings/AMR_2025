...
class OutdoorGuiNode(Node):
    ...
    def _build_gui(self):
        ...
        tk.Label(wp, text="Waypoints (Drag / Click)", font=lbl_font, fg=ACCENT, bg=BG_PANEL).pack(anchor="w")

        tools_frame = tk.Frame(wp, bg=BG_PANEL)
        tools_frame.pack(fill=tk.X, pady=2)
        
        self.btn_home = tk.Button(tools_frame, text="🏠 Home\n(Click)", font=lbl_font, bg=ACCENT2, fg=WHITE, cursor="hand2", command=lambda: self._toggle_tool('home'), bd=0)
        self.btn_home.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        
        self.btn_end = tk.Button(tools_frame, text="🚩 End\n(Click)", font=lbl_font, bg=ACCENT2, fg=WHITE, cursor="hand2", command=lambda: self._toggle_tool('end'), bd=0)
        self.btn_end.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        
        self.btn_chk = tk.Button(tools_frame, text="🏁 ChkPt\n(Click)", font=lbl_font, bg=ACCENT2, fg=WHITE, cursor="hand2", command=lambda: self._toggle_tool('checkpoint'), bd=0)
        self.btn_chk.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)

        self.btn_pickup = tk.Button(tools_frame, text="📦 PICKUP\n(Auto)", font=lbl_font, bg="#e67e22", fg=WHITE, cursor="hand2", command=self._start_pickup, bd=0)
    ...
    def _navigation_step(self):
        ...
        if self.latest_scan:
            ...
            if min_front < 1.5:
                ...
                v = 0.1  # slight forward motion to not get stuck
            else:
